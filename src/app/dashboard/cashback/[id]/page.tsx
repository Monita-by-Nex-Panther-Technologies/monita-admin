...
import { ArrowLeft, Check, Copy, Download, Filter, ListFilter, MoreHorizontal, Search, X } from "lucide-react";
...
const RecipientDetails = () => {
    ...
    const filteredTransactions = recipient.transactions.filter((transaction) => {
        ...
        const matchesUsername = filters.username
            ? (transaction as any).username?.toLowerCase().includes(filters.username.toLowerCase())
            : true;
        ...
        const matchesTransactionType = filters.transactionType
            ...
            : true;
        const matchesStartDate = filters.startDate instanceof Date
            ? new Date(transaction.date) >= filters.startDate
            : true;

        const matchesEndDate = filters.endDate instanceof Date
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-6">
            ...
            <div className="bg-white rounded-xl p-4 flex items-center justify-between mb-6 shadow-sm">
                <Link href="/dashboard/cashback" className="flex items-center text-lg font-semibold gap-2">
                    <ArrowLeft size={20} />
            ...
            <div className="bg-white rounded-xl shadow-sm mb-6 overflow-hidden">
                ...
                <div className="border-b p-6 flex items-center gap-4">
                    <Avatar className="h-14 w-14">
                        <AvatarImage src={`https://ui-avatars.com/api/?name=${recipient.name}&background=random`} />
                        <AvatarFallback>{recipient.name.substring(0, 2)}</AvatarFallback>
                    </Avatar>
                    <div>
                        <div className="flex items-center gap-2">
                            <span className="text-lg font-semibold">Image Verified</span>
                ...
                <CardContent className="p-0">
                    ...
                    <div className="grid grid-cols-4 p-6 pt-0">
                        ...
                        <div>
                            ...
                            <div className="font-medium text-blue-600">{recipient.device.ipAddress}</div>
                        </div>
                        <div>
                ...
                <CardContent className="p-0">
                    ...
                    <div className="grid grid-cols-4 p-6 pt-0">
                        ...
                        <div>
                            ...
                            <div className="font-medium">{recipient.otherDetails.statusCode}</div>
                        </div>
                        <div>
                            <div className="text-gray-500 mb-2">Process Time</div>
                            <div className="font-medium">{recipient.otherDetails.processTime}</div>
                        </div>
                        <div>
                            <div className="text-gray-500 mb-2">Provider details</div>
                            <div className="font-medium">{recipient.otherDetails.providerDetails}</div>
                        </div>
                    </div>
                </CardContent>
            </Card>
                            ...
                            <TableRow>
                                <TableHead className="p-4">
                                    <input
                                        ...
                                        checked={selectedTransactions.length === recipient.transactions.length && selectedTransactions.length > 0}
                        ...
                        <TableBody>
                            {paginatedData.map((transaction) => (
                                <TableRow key={transaction.id} className="py-6">
                                    ...
                                    <TableCell className="text-text-body font-poppins text-base py-6">{(transaction as any).username || '-'}</TableCell>
                                    ...
                                    <TableCell className="text-text-body font-poppins text-base py-6">{transaction.date}</TableCell>
                                    <TableCell>
