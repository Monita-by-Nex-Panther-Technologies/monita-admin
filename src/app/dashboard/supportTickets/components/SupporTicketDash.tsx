...
const SupportTicketTable = () => {
    ...
    const exportData = (format: string) => {
        const dataToExport =
            selected.length > 0 ? filteredTickets.filter((ticket) => selected.includes(ticket.id)) : filteredTickets

        if (format === "DOC") {
            const docContent = dataToExport.map((t) => `${t.name}\t${t.id}\t${t.issue}\t${t.status}`).join("\n")
            ...
            autoTable(doc, {
                ...
                body: dataToExport.map((ticket) => [ticket.name, ticket.id, ticket.issue, ticket.status]),
                startY: 30,
                styles: {
                    fontSize: 10,
                    cellPadding: 3,
                            ...
                            <TableRow>
                                <TableHead className="p-4">
                                    <input
                                        ...
                                        checked={selected.length === filteredTickets.length && selected.length > 0}
                                        onChange={handleSelectAll}
                                    />
                                </TableHead>
                                <TableHead className="text-base font-medium">Name</TableHead>
                                <TableHead className="text-base font-medium">Issue Summary</TableHead>
                                <TableHead className="text-base font-medium">Ticket Status</TableHead>
                        ...
                        <TableBody>
                            {paginatedData.map((ticket, index) => (
                                <TableRow key={index} className="py-6">
                                    ...
                                    <TableCell className="text-base py-6">{ticket.name}</TableCell>
                                    <TableCell className="text-base py-6">{ticket.issue}</TableCell>
                                    <TableCell className="text-base py-6">
                                        <span className="px-4 py-2 rounded-md bg-[#E6F7EF] text-[#00A85A]">{ticket.status}</span>
                                    </TableCell>
