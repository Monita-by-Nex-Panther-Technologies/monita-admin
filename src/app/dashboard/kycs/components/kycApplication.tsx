...
import { Search, MoreHorizontal, ListFilter, ChevronLeft, ChevronRight, Copy, X } from "lucide-react";
...
import { kycApplications, KycApplication } from "./data"; // Import data from data.ts
...
const KycApplicationsTable = () => {
    ...
    const filteredApplications = kycApplications.filter((application) => {
        ...
        const matchesUsername = filters.username
            ? application.username.toLowerCase().includes(filters.username.toLowerCase())
            : true;
    ...
    const exportData = (format: string) => {
        const dataToExport = selected.length > 0
            ? filteredApplications.filter((application) => selected.includes(application.id))
            : filteredApplications;

        if (format === "DOC") {
            const docContent = dataToExport.map(a => `${a.username}\t${a.kycLevel}\t${a.meansOfId}\t${a.kycStatus}\t${a.approvedDate}`).join("\n");
            ...
            autoTable(doc, {
                head: [['Username', 'KYC Level', 'Means of ID', 'KYC Status', 'Approved Date']],
                    ...
                    <div className="relative" ref={exportDropdownRef}>
                        ...
                        >
                            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <path d="M4.66667 6.66667L8 10L11.3333 6.66667" stroke="white" strokeWidth="1.33333" strokeLinecap="round" strokeLinejoin="round" />
                            ...
                            <TableRow>
                                <TableHead className="p-4">
                                    <input
                                        ...
                                        checked={selected.length === filteredApplications.length && selected.length > 0}
                                        onChange={handleSelectAll}
                                    />
                                </TableHead>
                                <TableHead className="text-base font-poppins text-text-title">Username</TableHead>
                                <TableHead className="text-base font-poppins text-text-title">KYC Level</TableHead>
                                <TableHead className="text-base font-poppins text-text-title">Means of ID</TableHead>
                                <TableHead className="text-base font-poppins text-text-title">KYC Status</TableHead>
                                <TableHead className="text-base font-poppins text-text-title">Approved Date</TableHead>
                        ...
                        <TableBody>
                            {paginatedData.map((application) => (
                                <TableRow key={application.id} className="py-6">
                                    ...
                                    <TableCell className="text-text-body font-poppins text-base py-6">{application.username}</TableCell>
                                    <TableCell className="text-text-body font-poppins text-base py-6">{application.kycLevel}</TableCell>
                                    <TableCell className="text-text-body font-poppins text-base py-6">{application.meansOfId}</TableCell>
                                    <TableCell className="text-text-body font-poppins text-base py-6">
                                        <span
                                            className={`px-2 py-1 rounded-md w-full ${application.kycStatus === "Approved"
                                                ? "text-green-500"
                                    ...
                                    <TableCell className="text-text-body font-poppins text-base py-6">{application.approvedDate}</TableCell>
                                    <TableCell>
